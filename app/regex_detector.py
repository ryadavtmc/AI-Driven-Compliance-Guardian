...
--------------------------------------------------------------------
- Detects common PII (email, phone, SSN, credit card, DOB) and secrets (AWS keys, GitHub tokens, Slack/Stripe keys, JWTs, DB URLs, private keys, etc.)
...
class PatternSpec:
    ...
    validate: Optional[str] = None  # name of validator function


# NOTE: Some patterns are context-agnostic and may produce false positives.
# Validators help reduce false positives for certain patterns.
...
def detect(text: str) -> List[Match]:
    """Return a list of matches found in text across all patterns."""
    findings: List[Match] = []
    for spec in PATTERNS:
        pattern = re.compile(spec.regex, spec.flags)
        for m in pattern.finditer(text):
            # Prefer full match; if groups exist, take the full string span
            matched_text = m.group(0)
            # Optional validation to reduce false positives
            if spec.validate:
                validator = _VALIDATORS.get(spec.validate)
                if validator:
                    # Choose a sensible string to validate: remove separators for CC num
                    cand = matched_text
    ...
    # De-duplicate overlapping identical spans by preferring higher severity
...
def mask_text(text: str, matches: List[Match], strategy: str = "redact") -> str:
    """Mask matches in text. Strategies: 'redact' (default) or 'partial' (for emails/phones/cc)."""
